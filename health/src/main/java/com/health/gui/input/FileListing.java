...
public class FileListing extends JPanel {
	...
	/**
	 * Represents an iterable of input data.
	 * @author lizzy
	 *
	 */
	public static class InputDataIterable implements Iterable<InputData> {
		private List<FileListingRow> fileListingRows;

		/**
		 * Create new InputDataIterable.
		 * @param fileListingRows	rows
		 */
		public InputDataIterable(final List<FileListingRow> fileListingRows) {
			this.fileListingRows = fileListingRows;
		}

		@Override
		public final Iterator<InputData> iterator() {
			return new InputDataIterator(fileListingRows);
		}
	}

	/**
	 * Represents an iterator that can iterate input data.
	 * @author lizzy
	 *
	 */
	public static class InputDataIterator implements Iterator<InputData> {
		private List<FileListingRow> fileListingRows;
		private int index;

		/**
		 * Create a new InputDataIterator.
		 * @param fileListingRows	rows
		 */
		public InputDataIterator(final List<FileListingRow> fileListingRows) {
			this.index = 0;
			this.fileListingRows = fileListingRows;
		}
		...
		public final boolean hasNext() {
			return index < fileListingRows.size();
		}

		@Override
		public final InputData next() {
			FileListingRow row = fileListingRows.get(index);

			InputData data = getInputData(row);
		...
		private InputData getInputData(final FileListingRow row) {
			String xmlFormat = row.getXmlFormat().getSelectedItem().toString();
			String fileString = row.getFileString();

			String name = "table" + index;

			xmlFormat = UserInterface.PATH_TO_CONFIG_XML + xmlFormat + ".xml";

			return new InputData(fileString, xmlFormat, name);
		}
	}

	/**
	 * Get an iterable that can iterate over the input data in the file listing.
	 * @return	iterable
	 */
	public static Iterable<InputData> getInputData() {
		return new InputDataIterable(fileListingRows);
	}
