...
public class OutputController {
	...
	public OutputController (VOutputPanel outputP){
		outputPanel = outputP;
	}
	...
	public void control(){
        ...
        outputPanel.getSidebar().getList().addMouseListener(new MouseListener() {

            @Override
            public void mouseClicked(final MouseEvent e) {
            	JList<String> list = outputPanel.getSidebar().getList();
            	if(list.getModel().getSize() > 0) {
	                String selected = outputPanel.getSidebar().getList()
	                        .getSelectedValue();
	                outputPanel.getMainPanel().setData(outputPanel.getSidebar()
