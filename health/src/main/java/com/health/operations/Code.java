...
public class Code {
  ...
  /**
   * Fills up an EventSequence object with EventLists with the codePatern declared in the
  ...
  public static List<EventList> fillEventSequence(final EventSequence eventSeq,
    ...
    while (currentIndex < eList.size()) {
      ...
      makeEventList: if (eventSeq.isConnected()) {
        ...
        currentIndex = findEventInSeq(codePattern[0], eventList, currentIndex);
        ...
        for (int i = 1; i < codePattern.length; i++) {
          nextIndex = findEventInSeq(codePattern[i], eventList, nextIndex);
   ...
   *          the EventList Object in which to look for the pattern.
   * @param per
   *          the maximum amount of time between the first event of the sequence and the last.
   */
  public static void fillEventSequenceWithinPeriod(final EventSequence eventSeq,
      final EventList eventList, final Period per) {
    fillEventSequence(eventSeq, eventList);

    deleteOutSideOfPeriod(eventSeq, per);
  }

  private static int findEventInSeq(final String code, final EventList eventList,
  ...
  private static void deleteOutSideOfPeriod(final EventSequence eventSeq, final Period per) {
