...
public final class Connect {
  ...
  /**
   * A function which connects two Table objects and return one table object ordered by date.
   *
   * @param table1
  ...
  public static Table connect(final Table table1, final Table table2,
    ...
    List<Column> connectedTableCols = makeNewTableColumns(table1.getColumns(), table2.getColumns(),
        connections);
    // make new Table
  ...
  private static List<Column> makeNewTableColumns(final List<Column> table1Cols,
      final List<Column> table2Cols, final List<ColumnConnection> connections) {
    ...
    for (Column column : table1Cols) {
      ...
      if (index >= 0) {
        result
            .add(new Column(connections.get(index).getNewName(), result.size(), column.getType()));
      } else {
   ...
   * @return the index of the the connection in the list. If not found return -1;
   */
  private static int isInConnections(final Column col, final List<ColumnConnection> connections) {
    int index = 0;
