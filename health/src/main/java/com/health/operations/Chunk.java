...
public final class Chunk {
   ...
   * @param dateColumn
   *          the column on which to chunk. The column must be a column of type Date.
   * @param operations
   ...
   */
  public static Table chunkByPeriod(final Table table, final String dateColumn,
   ...
   * @return a chunked Table.
   */
  public static Table chunkByColumn(final Table table, final String keyColumn,
  ...
  private static Map<Object, List<Record>> groupByPeriod(final Table table, final String column,
  ...
  private static void insertInGroup(final Map<Object, List<Record>> groups, final Object key,
      final Record record) {
  ...
  private static Table flattenGroups(final Table table, final String keyColumn,
      final List<ColumnAggregateTuple> operations, final Map<Object, List<Record>> groups) {
  ...
  private static Iterable<Entry<Object, List<Record>>> asSortedList(
    ...
    if (Double.class.isAssignableFrom(type)) {
      ...
      list.sort((a, b) -> ((LocalDateTime) a.getKey()).compareTo((LocalDateTime) b.getKey()));
    } else {
  ...
  private static double aggregate(final List<Record> records, final String column,
      final AggregateFunction<?> function) {
    return function.apply(records, column);
  }

  private static List<Column> createChunkTableColumns(final Table table, final String keyColumn,
      final List<ColumnAggregateTuple> operations) {
  ...
  private static List<Record> findRecordsInPeriod(final Table table, final String column,
      final LocalDateTime beginOfPer, final LocalDateTime endOfPer) {
